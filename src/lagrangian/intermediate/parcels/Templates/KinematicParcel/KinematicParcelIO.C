...
void Foam::KinematicParcel<ParcelType>::readFields(CloudType& c)
{
	...
	// CJC {
	IOField<vector> globalPos
	(
		c.fieldIOobject("globalPos", IOobject::MUST_READ),
		write
	);
	c.checkFieldIOobject(c, globalPos);
	// } CJC
...
void Foam::KinematicParcel<ParcelType>::writeFields(const CloudType& c)
{
	...
	IOField<vector> globalPos(c.fieldIOobject("globalPos", IOobject::NO_READ), np); // CJC
