...
#include "cloudSolutionSC.H"
#include "Time.H"
#include "localEulerDdtScheme.H"

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::cloudSolutionSC::cloudSolutionSC
(
    const fvMesh& mesh,
...
void Foam::cloudSolutionSC::read()
{
...
Foam::scalar Foam::cloudSolutionSC::relaxCoeff(const word& fieldName) const
{
    forAll(schemes_, i)
    {
...
bool Foam::cloudSolutionSC::semiImplicit(const word& fieldName) const
{
    forAll(schemes_, i)
    {
        if (fieldName == schemes_[i].first())
...
bool Foam::cloudSolutionSC::solveThisStep() const
{
    return active_ && (mesh_.time().timeIndex() % calcFrequency_ == 0);
}


bool Foam::cloudSolutionSC::canEvolve()
{
    if (transient_)
    {
...
bool Foam::cloudSolutionSC::output() const
{
    // CJC {
        if (LagrangianWrite_ == 0)
        {
