...
)
{
    ...
    if (prepareForNextTimeStep(time, newParcels, newVolumeFraction))
    {
		...
		// CJC {
		// Store current injector ID to improve ThirdPartyPlaneInjection efficiency
		label currentInjector = 0;
		// } CJC
        ...
        for (label parcelI = 0; parcelI < newParcels; parcelI++)
        {
            if (validInjection(parcelI))
            {
				...
				// CJC {
				if (this->modelType() == "ThirdPartyPlaneInjection")
				{
					setPositionAndCellTP
					(
						parcelI,
						newParcels,
						time,
                ...
                if (celli > -1)
                {
					...
					if (this->modelType() == "ThirdPartyPlaneInjection")
					{
	                    ...
	                    setPropertiesTP(currentInjector, *pPtr);
					}
					else
					{
	                    // Assign new parcel properties in injection model
				...
				if (this->modelType() == "ThirdPartyPlaneInjection")
				{
					label injectorCheck = celli;

					if (Pstream::parRun())
					{
						reduce(injectorCheck, maxOp<label>());

						if (injectorCheck == -1)
						{
							Info << nl << "WARNING: Injector " << currentInjector - 1 << " failed to initialise" << endl;
						}
					}
					else
					{
