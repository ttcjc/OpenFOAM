...
)
{
    ...
    // Clear the global positions as they are about to change
...
// CJC {
template<class ParticleType>
void Foam::Cloud<ParticleType>::extractionData(ParticleType& p) const
{
	std::ofstream extractionData
	(
		"extractionData/extractionData",
		ios_base::app
	);

	extractionData << p.globalPos().x() << " "
				   << p.globalPos().y() << " "
				   << p.globalPos().z() << " "
				   << p.U().x() << " "
				   << p.U().y() << " "
